...
class WebQSP(datasets.GeneratorBasedBuilder):
    ...
    def __init__(self, *args, writer_batch_size=None, **kwargs):
        ...
        self.mid2name_mappings = self.get_mids_to_name_mappings()
    ...
    def get_mids_to_name_mappings(self):
        file = "/mnt/jiangjinhao/PLM4KBQA/data/Freebase/FastRDFStore_data/mids2name.json"
        ...
        with open(data_filepath, encoding="utf-8") as f:
            for idx, line in enumerate(f):
                ...
                for answer in answers:
                    if answer[1] is None:
                        answers_new.append("None")
                    else:
                        answers_new.append(answer[1])
                kg_tuples = ex["kg_tuples"]
                kg_tuples_new = []
                for kg_tuple in kg_tuples:
                    h,r,t = kg_tuple
                    try:
                        h = self.mid2name_mappings[h][0]
                    except:
                        h = h
                    try:
                        t = self.mid2name_mappings[t][0]
                    except:
                        t = t
                    kg_tuples_new.append([h,r,t])
                yield idx, {
                    "id": ex["ID"],
                    "question": ex["question"],
                    "answers": answers_new,
                    "s_expression": ex["s_expression"],
                    "kg_tuples": kg_tuples_new,
